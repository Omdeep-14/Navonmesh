...
const MOOD_EMOJIS = {
  ...
  stressed: "😩", anxious: "😰", sad: "😢", angry: "😤", depressed: "😔", suicidal: "💜",
};
...
const MoodLineGraph = ({ history }) => {
  ...
  useEffect(() => {
    ...
    ctx.beginPath(); ctx.moveTo(xOf(0), yOf(points[0].score));
    for (let i = 0; i < points.length - 1; i++) {
      const p0 = points[Math.max(i - 1, 0)], p1 = points[i];
      const p2 = points[i + 1], p3 = points[Math.min(i + 2, points.length - 1)];
    ...
    points.forEach((p, i) => {
      const x = xOf(i), y = yOf(p.score), color = getMoodColor(p.mood).dot;
      ctx.beginPath(); ctx.arc(x, y, 4, 0, Math.PI * 2);
      ctx.fillStyle = color; ctx.shadowColor = color; ctx.shadowBlur = 10;
      ctx.fill(); ctx.shadowBlur = 0;
      ctx.fillStyle = "rgba(148,163,184,0.5)"; ctx.font = "10px Inter, sans-serif"; ctx.textAlign = "center";
      ctx.fillText(friendlyDay(p.day).split(",")[0], x, H - 5);
    });

    [["😊", 90], ["—", 50], ["💜", 10]].forEach(([t, s]) => {
      ctx.fillStyle = "rgba(100,116,139,0.5)"; ctx.font = "10px Inter, sans-serif";
      ctx.textAlign = "left"; ctx.fillText(t, 2, yOf(s) + 4);
    });
  }, [history]);
...
const Sidebar = ({ user, allMessages, activeView, setActiveView, onResetChat, onOpenDayChat }) => {
  ...
  return (
    <aside className="w-64 flex-shrink-0 flex flex-col h-full" style={{
      background: "rgba(8,12,25,0.75)", backdropFilter: "blur(20px)",
      WebkitBackdropFilter: "blur(20px)", borderRight: "1px solid rgba(255,255,255,0.06)",
    }}>
      <div className="px-5 py-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xl">🌙</span>
          <span className="text-base font-bold font-serif text-amber-300">Sahaay</span>
        </div>
      ...
      <div className="flex items-end gap-3">
        <div className="flex-1">
          <textarea
            ...
            placeholder="Share how you're feeling..." rows={1} className="glass-input"
            style={{ resize: "none", minHeight: "44px", maxHeight: "120px" }}
            onInput={(e) => { e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 120) + "px"; }}
          />
        </div>
        <button
          onClick={handleSend} disabled={!text.trim() || isTyping}
          className="w-11 h-11 rounded-2xl flex items-center justify-center font-bold flex-shrink-0 transition-all"
          style={{ background: "linear-gradient(135deg,#fbbf24,#fb7185)", color: "#0f172a", opacity: !text.trim() || isTyping ? 0.35 : 1 }}
  ...
  if (selectedDay) {
    ...
    return (
      <div className="flex-1 overflow-y-auto p-6">
        <button onClick={() => setSelectedDay(null)} className="flex items-center gap-2 text-sm mb-6 transition-colors" style={{ color: "#64748b" }}
    ...
    <div className="flex-1 overflow-y-auto p-6">
      ...
      {history.length > 0 && (
        <div className="rounded-2xl p-5 mb-6" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", backdropFilter: "blur(12px)" }}>
          ...
          <div style={{ overflowX: "auto" }}><MoodLineGraph history={history} /></div>
          <div className="flex flex-wrap gap-4 mt-4">
            {[{ mood: "happy", label: "Happy / Calm" }, { mood: "okay", label: "Okay" }, { mood: "anxious", label: "Anxious / Stressed" }, { mood: "sad", label: "Sad / Low" }].map(({ mood, label }) => (
      ...
      {dayKeys.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {dayKeys.map((day) => {
            ...
            return (
              <button key={day} onClick={() => setSelectedDay(day)} className="text-left rounded-2xl p-4 transition-all duration-300"
...
export default function Home({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    if (!token) { onNavigate("landing"); return; }
    ...
    const fetchReplyContext = async () => {
      try {
        ...
        const contextMsg = { id: Date.now(), role: "assistant", content: data.message || `${getGreeting()}, ${user?.name?.split(" ")[0] || "there"} 🌙 How are you feeling right now?`, timestamp: Date.now() };
        setChatOpen(true); setActiveView("chat");
        setMessages([contextMsg]); setAllMessages((prev) => [...prev, contextMsg]);
      } catch {
        const welcome = { id: Date.now(), role: "assistant", timestamp: Date.now(), content: `${getGreeting()}, ${user?.name?.split(" ")[0] || "there"} 🌙 How are you feeling right now?` };
  ...
  const handleSend = async (text) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/morning-checkin?fast=true`, {
        method: "POST",
  ...
  return (
    <div className="h-screen flex overflow-hidden" style={{ background: "radial-gradient(ellipse at 20% 50%, rgba(30,20,60,0.9) 0%, rgba(8,12,25,1) 70%)" }}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Header */}
        <header className="flex items-center justify-between px-6 py-4 flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(8,12,25,0.6)", backdropFilter: "blur(16px)" }}>
          <div className="flex items-center gap-3">
            {chatOpen && activeView === "chat" && (
              <button onClick={() => setChatOpen(false)} className="p-1.5 rounded-xl transition-all text-slate-600"
                onMouseEnter={(e) => { e.currentTarget.style.color = "#e2e8f0"; e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.color = "#64748b"; e.currentTarget.style.background = "transparent"; }}>←</button>
            )}
            <div>
              <h1 className="text-white font-semibold font-serif text-sm">
                {activeView === "history" ? "Mood History" : chatOpen ? "Chat with Sahaay" : "Dashboard"}
              </h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="mendi-btn mendi-btn-red mendi-btn-sm"><span>Sign out</span></button>
          </div>
        </header>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-10">
            <div className="max-w-xl mx-auto">
              <div className="mb-8">
                ...
                <p className="text-slate-500" style={{ fontWeight: 300 }}>How are you doing today? Sahaay is here whenever you're ready.</p>
              </div>

              <div className="grid grid-cols-3 gap-3 mb-8">
                {[{ label: "Check-ins", value: totalCheckins, icon: "💬" }, { label: "This week", value: weekCheckins, icon: "📅" }, { label: "Top mood", value: topMood, icon: "✨" }].map(({ label, value, icon }) => (
                  <div key={label} className="rounded-2xl p-4 text-center" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", backdropFilter: "blur(8px)" }}>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {history.slice(0, 4).map((entry) => (
                      <div key={entry.id} className="rounded-xl px-4 py-3 flex items-center gap-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", backdropFilter: "blur(8px)" }}>
